...
const I18N = {
  en: {
    ...
    'settings.subtitle': 'Configure US currency, number format, company details, and local app preferences.',
    'settings.countryProfile': 'Country / market profile',
...
function _settingsMerge(settings) {
  ...
  const locale = profilePreset
    ...
    : (SETTINGS_ALLOWED_LOCALES.includes(merged.locale) ? merged.locale : DEFAULT_SETTINGS.locale);
  ...
  delete merged.tax_label;
  delete merged.tax_id_label;
  delete merged.tax_rates;
  delete merged.tax_disclaimer;
  delete merged.tax_mode;
  return merged;
}
...
const AppSettings = {
  ...
  collectForm(prefix) {
    ...
    return {
      ...
      language,
      currency,
      locale: value('locale') || (profilePreset ? profilePreset.locale : DEFAULT_SETTINGS.locale),
      company: {
  ...
  async loadDemoData() {
    ...
    await DB.insertBatch('budget_costi', [
      { codice: 'PRJ-001', categoria: 'Labor', descrizione: 'Project work', qta: 80, um: 'h', costo_unitario: 45, importo: 3600, note: '' },
      { codice: 'PRJ-001', categoria: 'Materials', descrizione: 'Software and assets', qta: 1, um: 'lot', costo_unitario: 650, importo: 650, note: '' },
      { codice: 'PRJ-002', categoria: 'Subcontractors', descrizione: 'External contractor', qta: 1, um: 'lot', costo_unitario: 4200, importo: 4200, note: '' },
    ]);
    await DB.insertBatch('budget_ricavi', [
      { codice: 'PRJ-001', tipo_ricavo: 'Fixed fee', descrizione: 'Project fee', importo: 8200, note: '' },
      { codice: 'PRJ-002', tipo_ricavo: 'Quote', descrizione: 'Project fee', importo: 9800, note: '' },
    ]);
    await DB.insertBatch('consuntivo_costi', [
      { codice: 'PRJ-001', categoria: 'Labor', descrizione: 'Actual work', qta: 42, um: 'h', costo_unitario: 45, importo: 1890, note: '' },
    ]);
    await DB.insertBatch('consuntivo_ricavi', [
      { codice: 'PRJ-001', tipo_ricavo: 'Deposit', descrizione: 'First invoice', importo: 4100, note: '' },
    ]);
    await DB.insertBatch('cg_budget', [
      { voce: 'Software subscriptions', gen: 120, feb: 120, mar: 120, apr: 120, mag: 120, giu: 120, lug: 120, ago: 120, set: 120, ott: 120, nov: 120, dic: 120 },
      { voce: 'Accounting', gen: 180, feb: 180, mar: 180, apr: 180, mag: 180, giu: 180, lug: 180, ago: 180, set: 180, ott: 180, nov: 180, dic: 180 },
    ]);
    DB.invalidateCache();
    App.go('dashboard');
  },
